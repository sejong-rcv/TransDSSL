...
########################################################################################################################
    ...
    def _get_next_file(idx, file):
        ...
        return idx#os.path.join(os.path.dirname(file), idx)
    ...
    @staticmethod
    def _read_raw_calib_file(folder,idx):
        """Read raw calibration files from folder."""
        path=idx.split("/")
        name=path[-1].split("_")

        name=name[0]+"_"+name[1]+"_"+"000019"+"_"+"camera"+".json"
        jsonpath=os.path.join(folder,"camera",path[5],path[6],name)
        with open(jsonpath, 'r') as f:
            json_data = json.load(f)
        fx=json_data["intrinsic"]["fx"]
        fy=json_data["intrinsic"]["fy"]
        u0=json_data["intrinsic"]["u0"]
        v0=json_data["intrinsic"]["v0"]
...
########################################################################################################################
    ...
    def _get_sample_context(self, sample_name,
        ...
        f_=int(sp[1])
        s_=int(sp[2])
        front="{}_{:06d}_{:06d}_{}".format(place_,f_,s_-1,sp[-1])
        front=os.path.join(parent_folder,front+ext)
        back="{}_{:06d}_{:06d}_{}".format(place_,f_,s_+1,sp[-1])
        back=os.path.join(parent_folder,back+ext)
...
########################################################################################################################
    ...
    def __getitem__(self, idx):
        ...
        if parent_folder in self.calibration_cache:
            ...
            c_data = self._read_raw_calib_file(parent_folder,self.paths[idx])
